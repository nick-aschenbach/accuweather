...
describe Accuweather::Conditions::Local do
  ...
  describe 'accessors' do
    it 'allows accessing the initialized attributes' do
      ...
      expect(subject.latitude).to eq('39.89166')
      expect(subject.longitude).to eq('-105.7631')
  ...
  describe '#to_s' do
    it 'returns a string representation of the object' do
      expect(subject.to_s).to eq('city: Winter Park, state: Colorado, latitude: 39.89166, longitude: -105.7631, time: 21:14, time_zone: -7, obs_daylight: 0, current_gmt_offset: -7, time_zone_abbreviation: MST')
    end
  end
end